...
export type MicroAppAudio = MicroAppAudioEndpoint & {
  resume: () => void
  onRecovered?: (callback: (endpoint: MicroAppAudioEndpoint) => void) => () => void
}
...
function buildCollisionBitmaps(image: HTMLImageElement): CollisionBitmaps {
  ...
  for (let index = 0; index < pixelCount; index += 1) {
    ...
    const isLowerBrownBase =
      row >= baseStart && red - green > 20 && green >= blue && green - blue < 32 && red < 205

    if (isLightRope) {
      setBitmapPixel(scratchable, index)
    }
    if (isLowerBrownBase) {
  ...
  for (let row = baseStart; row < height; row += 1) {
    ...
    for (let column = left; column <= right; column += 1) {
      ...
      if (isBehindPostCore && row < centralBaseReveal) {
        continue
      }
...
function drawTalon(
  ...
  const cone = new Path2D('M20.9 18.55L12.9 2.57A1 1 0 0 0 11.1 2.57L3.1 18.55Z')
  context.fill(cone)
  context.stroke(cone)
  context.beginPath()
...
function createScratchAudio(audio?: MicroAppAudio) {
  ...
  const playMew = () => {
    ...
    envelope.connect(destination)
    scheduleSound([voice, vibrato], [voice, vibrato, vibratoDepth, envelope], now, duration)
  }
  ...
  const playScreech = (pitchScale = 1) => {
    ...
    voice.frequency.exponentialRampToValueAtTime(980 * normalizedPitchScale, now + 0.2)
    voice.frequency.exponentialRampToValueAtTime(390 * normalizedPitchScale, now + duration)
    overtone.type = 'square'
    overtone.frequency.setValueAtTime(1_240 * normalizedPitchScale, now)
    overtone.frequency.exponentialRampToValueAtTime(780 * normalizedPitchScale, now + duration)
    ...
    scheduleSound(
      ...
      [voice, overtone, vibrato, vibratoDepth, overtoneGain, filter, envelope],
      now,
      duration
    )
  }
  ...
  const playRoar = (pitchScale = 1) => {
    ...
    growl.frequency.exponentialRampToValueAtTime(44 * normalizedPitchScale, now + duration)
    growlGain.gain.value = 0.18
    filter.type = 'lowpass'
    filter.frequency.setValueAtTime(520 * normalizedPitchScale, now)
    filter.frequency.exponentialRampToValueAtTime(180 * normalizedPitchScale, now + duration)
    filter.Q.value = 1.4
    envelope.gain.setValueAtTime(0.0001, now)
    ...
    filter.connect(envelope)
    envelope.connect(destination)
    scheduleSound([noise, growl], [noise, growl, growlGain, filter, envelope], now, duration)
  }
...
function createScratchPostApp(): MicroApp {
  ...
  return {
    mount(host) {
      if (mounted) {
        throw new Error('Scratch Post is already mounted')
      }
      ...
      const automaticTwinesPerSecond = () =>
        autoScratcherDefinitions.reduce(
          (total, definition) =>
            total + definition.twinesPerSecond * autoScratchers[definition.id].actors.length,
          0
        )
      ...
      const recordTwinesScratched = (count = 1) => {
        const validCount = Math.max(
          0,
          Math.min(Number.MAX_SAFE_INTEGER - twinesScratched, Math.floor(count))
        )
        if (!validCount) {
          return
        }
        ...
        try {
          ...
          if (
            autoScratcherDefinitions.every(
              (definition) =>
                autoScratchers[definition.id].actors.length === autoScratcherCounts[definition.id]
            ) &&
      ...
      const findBaseContact = (
        ...
        sampleXs.forEach((sampleX) => {
          ...
          for (let sourceY = startY; sourceY <= endY; sourceY += 1) {
            if (!hasBitmapPixel(base, sourceX, sourceY)) {
              continue
            }
            const surfaceY = layout.y + sourceY * sourceScale
            if (!earliestContact || surfaceY < earliestContact.surface) {
              earliestContact = { surface: surfaceY, sourceX, sourceY }
            }
            break
          }
        })
      ...
      const chooseLandingSourceY = (sourceX: number, firstSourceY: number) => {
        ...
        return candidates[
          Math.min(candidates.length - 1, Math.floor(candidates.length * interiorProgress))
        ]
      }
      ...
      const advanceHumanHandSweep = (timestamp: number, layout: ImageLayout) => {
        ...
        if (pose.contact) {
          ...
          twines.forEach((twine) => {
            ...
            const projection =
              segmentLengthSquared > 0
                ? Math.max(
                    ...
                    Math.min(
                      1,
                      ((twine.x - previousX) * segmentX + (twine.y - previousY) * segmentY) /
            ...
            const collisionDistance = Math.hypot(twine.x - closestX, twine.y - closestY)
            if (collisionDistance > contactRadius + twine.length * 0.5) {
              return
            }
            ...
            if (twine.settled) {
              ...
              twine.velocityX = pose.direction * (90 + Math.random() * 130)
              twine.velocityY = 70 + Math.random() * 100
              twine.angularVelocity = pose.direction * (4 + Math.random() * 8)
            } else {
              twine.velocityX += pose.direction * (75 + Math.random() * 110)
              twine.velocityY += Math.random() * 80 - 20
              twine.angularVelocity += pose.direction * (3 + Math.random() * 7)
            }
          })
        }
      ...
      const updateTwines = (timestamp: number, layout: ImageLayout) => {
        ...
        twines = twines.filter((twine) => {
          if (timestamp - twine.bornAt >= twine.lifetime) {
            return false
          }
          if (!elapsed || twine.settled) {
            return true
          }
          ...
          } else if (twine.landingSourceY === undefined && !twine.swept) {
            const baseContact = findBaseContact(twine, previousY, radius, layout)
            if (baseContact) {
              twine.landingSourceY = chooseLandingSourceY(baseContact.sourceX, baseContact.sourceY)
              twine.velocityX = 0
      ...
      const drawTwineCounter = () => {
        if (!twinesScratchedLoaded || !scratchCounterActive) {
          return
        }
      ...
      const drawAutoScratcherOptions = () => {
        ...
        let y = 40
        autoScratcherDefinitions.forEach((definition) => {
          if (twinesScratched < definition.cost) {
            return
          }

          const state = autoScratchers[definition.id]
          const actorCount = state.actors.length
          const label = actorCount > 0 ? `${definition.label} ×${actorCount}` : definition.label
      ...
      const shedTwine = (
        ...
        if (!imageReady || distance <= 0) {
          return false
        }
        if (!createParticle || twines.length >= maximumTwines) {
          if (countWhenParticleLimitReached) {
        ...
        twines.push({
          ...
          angle: Math.atan2(movementY, movementX) + (Math.random() * 0.7 - 0.35) * variation,
      ...
      const randomScratchSourcePoint = () => {
        ...
        for (let attempt = 0; attempt < 40; attempt += 1) {
          ...
          if (!hasBitmapPixel(scratchable, sourceX, sourceY)) {
            continue
          }
      ...
      const automaticScratchPose = (
        ...
        const offsetY =
          Math.sin(timeSeconds * actor.positionAngularSpeed * 0.45 + actor.positionPhaseY) *
          scratchable.height *
        ...
        if (!validAtScale(amplitudeScale)) {
          ...
          for (let iteration = 0; iteration < 10; iteration += 1) {
            ...
            if (validAtScale(candidateScale)) {
        ...
        const targetAngle =
          actor.baseAngle +
          Math.sin(timeSeconds * actor.angleAngularSpeed + actor.anglePhase) * actor.angleAmplitude
        if (
      ...
      const advanceAutoScratcherCountdowns = (timestamp: number) => {
        const elapsedSeconds = Math.max(0, (timestamp - lastAutomaticProgressAt) / 1000)
      ...
      const drawAutoScratcherPaws = (timestamp: number, layout: ImageLayout) => {
        ...
        autoScratcherDefinitions.forEach((definition) => {
          ...
          const strokeDistance = Math.max(13, Math.min(24, layout.width * 0.065)) * 3
          const pawScale = Math.max(0.62, Math.min(0.9, layout.width / 360)) * 2
          ...
          const visualPixelsPerSecond = Math.min(
            ...
            Math.max(
              ...
              (70 + Math.log1p(definition.twinesPerSecond) * 18) * levelSpeedMultiplier
            )
          )
          ...
          const phaseOffset = (visualActor.positionPhaseX / (Math.PI * 2)) * cycleSeconds
          ...
          const scratchProgress =
            cycleElapsed >= travelSeconds ? 0 : 0.5 - Math.cos(travelProgress * Math.PI * 2) * 0.5
          const centerToeDistance = pawCenterToeDistance * pawScale
          const scratchOffset = strokeDistance * (scratchProgress - 0.5)
          const pawX = pose.x - Math.cos(pose.angle) * (centerToeDistance - scratchOffset)
          const pawY = pose.y - Math.sin(pose.angle) * (centerToeDistance - scratchOffset)
      ...
      const emitDueAutoScratches = (
        ...
        autoScratcherDefinitions.forEach((definition) => {
          ...
          let dueCount = 0
          state.actors.forEach((actor) => {
            if (actor.countdownSeconds > 0) {
              return
            }

            const actorDueCount = Math.floor(-actor.countdownSeconds / intervalSeconds) + 1
        ...
        if (availableParticles === totalDue) {
          ...
          for (let slot = 0; slot < availableParticles; slot += 1) {
            const batch = dueBatches
              .filter((candidate) => particleAllocations[candidate.definition.id] < candidate.count)
              .sort(
                (left, right) =>
        ...
        dueBatches.forEach((batch) => {
          ...
          for (let emission = 0; emission < particleCount; emission += 1) {
            if (emitAutomaticTwine(batch.visualActor, timestamp, layout!, true)) {
              recordedParticles += 1
            }
          }
      ...
      const draw = (timestamp: number) => {
        ...
        if (inputActive || twines.length || hasAutoScratchers()) {
          requestDraw()
        }
      }
      ...
      const emitTwine = (
        ...
        if (shedTwine(x, y, movementX, movementY, elapsedMilliseconds, distance, timestamp)) {
      ...
      const buyUpgrade = (id: PurchaseId) => {
        ...
        if (!definition || twinesScratched < definition.cost) {
          return
        }
        ...
        if (id === 'clawless-bapper') {
          scratchAudio.playThump()
        } else if (id === 'kitty-claws') {
          scratchAudio.playMew()
        } else if (id === 'tiger-claws') {
          scratchAudio.playRoar(1.3)
        } else if (id === 'bear-claws') {
          scratchAudio.playRoar(0.78)
        } else if (id === 'raptor-talon') {
          scratchAudio.playScreech(1.35)
        } else {
          scratchAudio.playScreech(0.42)
        }
        requestDraw()
      }
      ...
      const recordMovement = (event: PointerEvent) => {
        ...
        if (!previous || distance < 0.5 || timestamp - previous.timestamp > inputHoldMilliseconds) {
          return
        }
      ...
      suspendApp = () => {
        ...
        if (animationFrame) {
          host.cancelFrame(animationFrame)
        }
        animationFrame = 0
        surface.style.cursor = originalCursor
        resetPointer()
    ...
    activate() {
      if (!mounted) {
        throw new Error('Scratch Post must be mounted before activation')
      }
      activateApp()
    },
    ...
    resize(viewport) {
      if (!mounted) {
        throw new Error('Scratch Post must be mounted before resize')
      }
      resizeApp(viewport)
    },
